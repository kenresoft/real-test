...
- This is a pnpm workspace package. Run `pnpm install` at the **repo root** first. It won't install
...
| `OWNER_RECOVERY_SECRET` | Optional, opt-in | Enables the break-glass `POST /api/v1/system/recover-owner` route. It is absent by default, and the route 404s until you set it. Treat it as a standing master key; see `docs/DEPLOYMENT.md`'s recovery section. |
...
with `SameSite=None; Secure` unconditionally. This is deliberate, not a bug: the Admin Worker is a
...
**full repository**, not a subdirectory. This is deliberate. Cloudflare's own docs
